...
### 📰 Hot News 国际新闻（4 个）
- **联合早报** - 🇸🇬 新加坡权威中文媒体（国内访问自动跳转中国版）
- **RT** - 🇷🇺 俄罗斯国际新闻（自动翻译）
- **联合国新闻** - 🇺🇳 联合国官方中文新闻
- **美联社 AP News** - 🇺🇸 美国权威新闻（自动翻译）
...
| 新闻源 | 国家/地区 | 语言 | 特点 |
|--------|----------|------|------|
| **联合早报** | 🇸🇬 新加坡 | 中文 | 东南亚权威华人媒体，国内访问自动跳转中国版 |
| **RT** | 🇷🇺 俄罗斯 | 英文→中文 | 俄罗斯国际视角，自动翻译 |
| **联合国新闻** | 🇺🇳 联合国 | 中文 | 联合国官方立场，权威发布 |
| **美联社 AP News** | 🇺🇸 美国 | 英文→中文 | 美国主流视角，自动翻译 |

### 使用示例

```bash
# 搜索国际新闻（自动翻译英文源）
python3 search_skill.py "俄乌冲突" news

# 搜索多个新闻源
python3 search_skill.py "中美贸易" news_zaobao,news_apnews

# 获取热点新闻
python3 search_skill.py "general" hotnews
```

### 自动翻译功能

- ✅ 英文新闻源（RT、AP News）自动翻译成中文
- ✅ 内置新闻关键词中英映射表（战争/原油/股市/科技等）
- ✅ 无需额外 API，直接抓取 + 翻译
...
# 📰 看国际新闻（Hot News）
python3 search_skill.py "俄乌冲突" news
python3 search_skill.py "中美贸易" news
```
...
```bash
# 主 Agent 判断需要子 Agent，询问用户后
# spawn 子 Agent 执行
sessions_spawn task="并行搜索多个数据源：股票行情 + 资金流 + 龙虎榜"
  timeoutSeconds=300
  streamTo="parent"
```
